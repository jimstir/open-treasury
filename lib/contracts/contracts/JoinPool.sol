...
contract JoinPool {
    ...
    function voting(uint256 proposal) internal virtual returns(bool){
        return TreasuryVault(_treasury).vote(proposal);
    }
    ...
    function approvePolicy() public virtual returns(bool){
        ...
        uint256 proposal = ITreasuryVault(_treasury).proposalOpen(_token, 0, address(0) );
        _policyNum = proposal;
    }
    ...
    function policyApproved() public virtual returns(bool){
        ...
        bool res = TreasuryVault(_treasury).proposalApproved(_policyNum);
        return res;
    }
    ...
    function fundsReq(address pool, uint256 time, uint256 amount, IERC20 token)public virtual returns(address){
        require(verifyPool(pool));
